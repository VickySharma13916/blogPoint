...
import webinar1image from "../../assets/images/webinar1image.png";
import webinarid from "../../assets/images/webinarid.jpg";
import useWindowToTop from "../../hooks/useWindowToTop";
import { performGetWebinarDetail } from "../../redux/actionCreators/webinarCreators";
import { BACKEND_URL } from "../../services/constant";
import DisplayText from "../DisplayHtmlText";
import RegisterForm from "../RegisterForm";
import Loading from "../Loading";

const WebinarTabInfo = ({ handleWebinarDetail }) => {
  useEffect(() => {
    useWindowToTop();
  }, []);
  ...
  const fetchData = async () => {
    const result = await handleWebinarDetail(id);
            ...
            <>
              <Helmet>
                ...
                {data?.webinarImage && (
                  <meta
                    property="og:image"
                    content={BACKEND_URL + data?.webinarImage}
                  />
                )}
                ...
                <div className="relative">
                  <div className="xl:ml-28 relative z-10">
                    <div className="border-0 relative active:border-0">
                      ...
                      <img
                        src={
                          data?.webinarImage
                            ? BACKEND_URL + data?.webinarImage
                            : webinarid
                        }
                        alt={data?.webinarName}
                      ...
                      <div className="flex flex-col text-white absolute top-0 bottom-0 right-0 left-0 z-10 text-left p-6 md:p-20">
                        <h1 className="text-3xl md:text-5xl w-full md:w-1/2 font-semibold webinartab">
                          {data?.webinarName && data?.webinarName}
                        </h1>
                        {data?.WebinarOtherLinks && (
                          <div className="flex md:flex-row flex-col md:gap-4 py-8">
                            {data?.WebinarOtherLinks.some(
                              (link) => link?.webinarOtherName === "Register"
                            ) ? (
                              <div
                            ...
                            {data?.WebinarOtherLinks.map(
                              (link) =>
                                link?.webinarOtherName !== "Register" && (
                                  <div
                                    key={link._id}
                                    className="text-red-800 w-fit md:text-lg border-2 border-red-800 font-semibold bg-white py-1 px-4"
                                  >
                                    <Link to={link?.WebinarOtherLinks}>
                                      <h4>{link?.webinarOtherName}</h4>
                                    </Link>
                                  </div>
                                )
                            )}
                  ...
                  <div className="container mx-auto">
                    <div className="my-12 md:my-20 mx-6 md:mx-0">
                      <img
                        src={
                          data?.webinarTimeLineImage
                            ? BACKEND_URL + data?.webinarTimeLineImage
                            : webinar1image
                        }
                        alt="webinar timeline image"
                      />
                    </div>
                  </div>
                  ...
                  <div className="w-full">
                    ...
                    {data?.joinWebinarImageSlider?.length > 0 && (
                    ...
                    <div className="container mx-auto">
                      {data?.trainerProfiles?.length > 0 && (
                        <>
                          ...
                          <div className="flex mx-6 md:mx-0 flex-wrap py-12 justify-center md:justify-start gap-12">
...
const mapDispatchToProps = (dispatch) => ({
  handleWebinarDetail: (payload) => dispatch(performGetWebinarDetail(payload)),
});

export default connect(mapStateToProps, mapDispatchToProps)(WebinarTabInfo);
