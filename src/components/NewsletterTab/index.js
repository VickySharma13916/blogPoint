...
import { performGetNewsletterList } from "../../redux/actionCreators/newsletterCreators";
import Pagination from "../pagination";
import { BACKEND_URL } from "../../services/constant";
import { setCurrentPage } from "../../redux/actions/newsletterActions";
...
const Newslettertab = ({
  ...
  const fetchNewsletter = async () => {
    const result = await fetchNewsletterList(`?pageNumber=${currentPage}`);
    if (result.status === 200) {
      const filtereData = result?.data?.docs?.data?.filter(
        (item) => item.status === true
      );
      setNewsletter(filtereData);
    ...
    <div className=" text-gray-800 mt-4 py-4 ">
      <div className="flex flex-wrap items-center justify-center md:justify-around py-10">
        {newsletter &&
          newsletter?.map((row) => (
            <Link
              to={`/newsletter/${row?.NewsletterLink?.toLowerCase()?.replace(
                /\s+/g,
                "-"
              )}`}
              key={row._id}
            >
              <div className=" card object-cover border-0 p-0 my-2 overflow-hidden flex flex-col">
                <img
                  src={BACKEND_URL + row.NewsletterImage}
                  alt={"Blog-releases"}
                ...
                <div className="flex bg-white h-full justify-between rounded-b-lg flex-col p-4">
                  ...
                  <div className="text-sm line-clamp-4">
                    <DisplayText htmlContent={row?.NewsletterDescription} />
                  </div>
...
const mapStateToProps = (state) => ({
  currentPage: state.newsletter.currentPage,
});

const mapDispatchToProps = (dispatch) => ({
  fetchNewsletterList: (payload) => dispatch(performGetNewsletterList(payload)),
  setCurrentPage: (page) => dispatch(setCurrentPage(page)),
});

export default connect(mapStateToProps, mapDispatchToProps)(Newslettertab);
